...
function TutorPerWsAssignmentRow({studentId, assignment, open, onToggle, regradeBusy, regradeResults, onRegrade}){
  const perWs = useWorksheetSubmissions(studentId, assignment.id);
  const worksheets = (assignment.worksheets || []).filter(w => !w.deleted);
  const docs = Object.values(perWs.byWorksheet || {});
  const submittedOrGraded = docs.filter(d => d && (d.status === "submitted" || d.status === "graded"));
  if(submittedOrGraded.length === 0){
    return null; // nothing to show for assignments with no per-WS submissions
  }
  return (
    <div style={{padding:"10px 12px",background:"#FAF7F2",border:"1px solid rgba(15,26,46,.08)",borderRadius:6}}>
      <div onClick={onToggle} style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",cursor:"pointer",gap:8,flexWrap:"wrap"}}>
        <div>
          ...
          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#66708A",letterSpacing:.5,textTransform:"uppercase"}}>
            {submittedOrGraded.length} per-WS submission{submittedOrGraded.length===1?"":"s"}
          </div>
        </div>
      ...
      {open && (
        <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:8}}>
          {worksheets.map(w=>{
            ...
            if(!d || (d.status !== "submitted" && d.status !== "graded")) return null;
            const busyKey = `${assignment.id}|${w.id}`;
            const busy = !!regradeBusy[busyKey];
            const result = regradeResults[busyKey];
            const isGraded = d.status === "graded" || typeof d.scoreCorrect === "number";
            const score = isGraded ? `${d.scoreCorrect}/${d.scoreTotal}` : "—";
            return (
              <div key={w.id} style={{padding:"8px 10px",background:"#fff",border:"1px solid rgba(15,26,46,.08)",borderRadius:4,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                <div style={{flex:"1 1 200px",minWidth:0}}>
                  <div style={{fontFamily:"'Fraunces',Georgia,serif",fontSize:13,color:"#0F1A2E",fontWeight:600}}>
                    {w.title}
                  </div>
                  <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#66708A",letterSpacing:.3,marginTop:2}}>
                    {d.status} · score {score}{d.gradeSkipReason ? ` · ${d.gradeSkipReason}` : ""}
                  </div>
                </div>

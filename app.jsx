...
function LatestPsmCard({student, studentId, submissions, canEdit}){
  ...
  const latest = useMemo(()=>{
    ...
    // then keeps insertion-order DESC as the tie-breaker — fixes the
    // case where a new PSM with today's date was being hidden behind
    // an older PSM that also had today's date (added earlier).
    const sorted = [...list].reverse().sort((a,b)=>{
...
function SingleWorksheetEditor({studentId, assignment, worksheetId, readOnly, onClose}){
  ...
  const writeDraft = async (overrideAnswers, overrideFlags) => {
    ...
    const myWrite = pendingWriteRef.current.then(async ()=>{
      ...
      try{
        // Session 18C v16: NEVER overwrite status if the doc is already
        // submitted/graded. previously writeDraft included status:'draft'
        // on every set, which (with merge:true) would clobber 'submitted'
        // back to 'draft' if writeDraft raced past submit. transaction
        // reads the current state and skips the status field when the
        // doc is locked, so the grade record can't be reverted by a
        // stale write.
        const FV = firebase.firestore.FieldValue;
        await window.db.runTransaction(async (tx) => {
          const ref = col.doc(currentWsId);
          ...
          if (!curData) {
            // Brand-new doc — set createdAt + initial draft status.
            payload.createdAt = FV.serverTimestamp();
            payload.status = "draft";
          } else if (curData.status === "draft" || !curData.status) {
            // Still in draft → reaffirm. Safe to keep at draft.
            payload.status = "draft";
          }
          // If status is "submitted" or "graded", do NOT include status
          // in payload — keep the locked state intact.
  ...
  // not started). Previously only submitted got a banner; drafts were
  ...
  if(localStatus === "submitted"){
    if(typeof effectiveScoreCorrect === "number" && typeof effectiveScoreTotal === "number"){
      ...
      banner = { text: "Submitted · Score pending… (refresh in a moment)", color: "#66708A", border: "rgba(102,112,138,.35)" };
    }
  } else if(doc && doc.status === "draft" && (doc.responses || []).some(r => (r.studentAnswer||"").trim() !== "")){
    banner = { text: "In progress · saved as draft (not yet submitted)", color: "#9A5B1F", border: "rgba(154,91,31,.4)" };
  }
...
// Session 18C v14: tutor view of per-worksheet submissions (the new
// per-WS path). Shows each per-WS doc per assignment with status,
// score, and a Re-grade button that calls regradeWorksheetSubmission.
// Use when a student reports "my answers look correct but it marked
// me wrong" — Re-grade re-runs the grader fresh and writes the audit
// trail to cloud function logs.
function TutorPerWsSection({student}){
  ...
  return (
    <div style={{...CARD, padding:"20px 22px", marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,fontWeight:600,letterSpacing:1.4,color:"#66708A",textTransform:"uppercase"}}>
          Per-Worksheet Submissions
        </div>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#66708A",fontStyle:"italic",letterSpacing:.3}}>
          Re-grade pulls fresh keys + writes audit to cloud logs
        </div>
      </div>
      {assignments.length === 0 ? (
        <div style={{fontFamily:"'Fraunces',Georgia,serif",fontStyle:"italic",fontSize:13,color:"#66708A"}}>
          No assignments on this student.
        </div>
      ) : (
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {assignments.slice().reverse().map(asg=>(
            <TutorPerWsAssignmentRow
              key={asg.id}
              studentId={student.id}
              assignment={asg}
              open={openAsgId === asg.id}
              onToggle={()=> setOpenAsgId(openAsgId === asg.id ? null : asg.id)}
              regradeBusy={regradeBusy}
              regradeResults={regradeResults}
              onRegrade={async(wsId)=>{
                if(!window.firebase || !window.firebase.app){ alert("Firebase not loaded"); return; }
                const key = `${asg.id}|${wsId}`;
                setRegradeBusy(prev=>({...prev,[key]:true}));
                try{
                  const fn = window.firebase.app().functions("us-central1").httpsCallable("regradeWorksheetSubmission");
                  const result = await fn({sid: student.id, aid: asg.id, wsId});
                  setRegradeResults(prev=>({...prev, [key]: result.data}));
                  console.log("[regrade]", key, result.data);
                }catch(e){
                  console.warn("[regrade] failed:", e);
                  alert("Regrade failed: " + (e.message||e));
                } finally {
                  setRegradeBusy(prev=>({...prev,[key]:false}));
                }
              }}
            />
          ))}
...
function TutorPerWsAssignmentRow({studentId, assignment, open, onToggle, regradeBusy, regradeResults, onRegrade}){
  ...
  for(const w of worksheets){
    ...
    if(!d) counts["not-started"]++;
    else if(d.status === "graded" || typeof d.scoreCorrect === "number") counts.graded++;
    else if(d.status === "submitted") counts.submitted++;
    else counts.draft++;
  }
  function statusFor(d){
    if(!d) return { kind: "not-started", color: "#66708A", border: "rgba(102,112,138,.35)", label: "Not started" };
    if(d.status === "graded" || typeof d.scoreCorrect === "number") {
      return { kind: "graded", color: "#4C7A4C", border: "rgba(76,122,76,.4)", label: `Graded · ${d.scoreCorrect}/${d.scoreTotal}` };
    }
    if(d.status === "submitted") return { kind: "submitted", color: "#003258", border: "rgba(0,50,88,.4)", label: "Submitted · pending grade" };
    return { kind: "draft", color: "#9A5B1F", border: "rgba(154,91,31,.4)", label: "In progress (draft)" };
  }
  return (
    <div style={{padding:"10px 12px",background:"#FAF7F2",border:"1px solid rgba(15,26,46,.08)",borderRadius:6}}>
      <div onClick={onToggle} style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",cursor:"pointer",gap:8,flexWrap:"wrap"}}>
        <div>
          <div style={{fontFamily:"'Fraunces',Georgia,serif",fontSize:14,fontWeight:600,color:"#0F1A2E"}}>
            {assignment.date || assignment.dateAssigned || "Undated"}
          </div>
          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#66708A",letterSpacing:.5,textTransform:"uppercase",display:"flex",gap:8,flexWrap:"wrap",marginTop:2}}>
            {counts.graded > 0 && <span style={{color:"#4C7A4C",fontWeight:600}}>✓ {counts.graded} graded</span>}
            {counts.submitted > 0 && <span style={{color:"#003258",fontWeight:600}}>⏳ {counts.submitted} pending</span>}
            {counts.draft > 0 && <span style={{color:"#9A5B1F",fontWeight:600}}>… {counts.draft} draft</span>}
            {counts["not-started"] > 0 && <span style={{color:"#66708A"}}>○ {counts["not-started"]} not started</span>}
          </div>
        </div>
        <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#003258",letterSpacing:.5,textTransform:"uppercase",fontWeight:600}}>
          {open ? "Hide ▾" : "Show ▸"}
        </span>
      </div>
      {open && (
...
function TutorSubmissionsPanel({student}){
  ...
  if(!submissions.length){
    // Session 18C v14: even with zero legacy submissions, the student
    // may have per-WS submissions (the new path). Render the per-WS
    // section above the empty state so per-WS data is reachable.
    return (
      <div>
        <TutorPerWsSection student={student}/>
        <div style={{...CARD, padding:"60px 40px", textAlign:"center"}}>
          <div style={{fontFamily:"'Fraunces',Georgia,serif",fontStyle:"italic",fontSize:20,color:"#66708A",letterSpacing:-.2}}>No legacy submissions on this student.</div>
          <div style={{fontSize:12,color:"#66708A",marginTop:10,lineHeight:1.55}}>Per-worksheet submissions (the new path) appear above if any exist.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Session 18C v14: per-WS submissions section. Lists each per-WS
          doc with a Re-grade button. Renders ONLY if there's per-WS
          data — invisible for legacy whole-PSM-only students. */}
      <TutorPerWsSection student={student}/>

...
function AppInner({authUser, onSignOut, currentUserEntry}){
  ...
  // ── Session 18C: Consultation Student seed (create-only) ──
  // Seeds the synthetic demo profile ONLY if missing. Previously we
  // auto-reseeded on version bump, but that overwrites any edits the
  // tutor made while demoing, AND triggered listener fan-out that
  // could clobber other in-progress inputs. Manual reset is provided
  // via the "Reset demo data" button in Students-tab admin actions
  // (added in this same commit). Once seeded per session,
  // _consultationSeeded prevents the effect from re-running.
  const _consultationSeeded = useRef(false);
  useEffect(()=>{
    if(_consultationSeeded.current) return;
    if(cloudStatus !== "synced") return; // wait for first listener tick
    const existing = students.find(st => st && st.id === CONSULTATION_STUDENT_ID);
    if(existing) {
      _consultationSeeded.current = true;
      return;
    }
    const col = studentsCollection();
    if(!col) return;
    _consultationSeeded.current = true; // mark before async to dedupe
    const data = makeConsultationStudent();
    col.doc(CONSULTATION_STUDENT_ID).set({...data, id: CONSULTATION_STUDENT_ID, _dataVersion: CONSULTATION_DATA_VERSION})
      .then(()=> console.log(`[consultation] seeded v${CONSULTATION_DATA_VERSION}`))
      .catch((e)=> {
        _consultationSeeded.current = false; // allow retry next mount
        console.warn("[consultation] seed failed:", e);
      });
  ...
  const savePreAssign=()=>{
    ...
    assignToWise(profile.id, entry.id, showToast);
  };
    ...
    <div style={{fontFamily:"'IBM Plex Sans',system-ui,sans-serif",background:"var(--paper)",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <div data-psm-header style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexShrink:0,gap:24}}>
        ...
        <div data-psm-actions style={{display:"flex",alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>
          ...
          <label title="Import students from a Wise Learner Report CSV (manual export)" style={{cursor:"pointer"}}>
            Import Wise CSV
          ...
          {/* Session 18: Wise API reconcile — checks each existing student
              against Wise via email lookup. Surfaces who's missing in
              Wise or has mismatched email. Does NOT bulk-import (the
              Wise API doesn't expose a 'list all students' endpoint;
              CSV remains the bulk-import path). Admin-only function. */}
          <button onClick={async()=>{
            if(!window.firebase || !window.firebase.app){ alert("Firebase not loaded"); return; }
            try{
              showToast("Reconciling with Wise…");
              const fn = window.firebase.app().functions("us-central1").httpsCallable("reconcileStudentsWithWise");
              const result = await fn({});
              const r = result.data || {};
              const t = r.totals || {};
              const lines = [
                `Wise reconcile complete:`,
                `  ${t.matched||0} matched in Wise`,
                `  ${t.unmatched||0} not found in Wise`,
                `  ${t.emailMismatched||0} email mismatched`,
                `  ${t.noEmail||0} no email on record`,
                `  ${t.errors||0} errors`,
              ];
              alert(lines.join("\n"));
              console.log("[wise-reconcile]", r);
            }catch(e){
              console.warn("[wise-reconcile] failed:", e);
              alert("Wise reconcile failed: " + (e.message||e));
            }
      ...
      {answerDisplay && answerDisplay.kind === "nested" && (
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {Object.keys(answerDisplay.groups).map(wId => {
            ...
            // entries can be sparse (gaps from out-of-order writes) — iterate by length
            const wsObj = (Array.isArray(submission.responses) ? submission.responses : [])
              .find(r => r && r.worksheetId === wId);
            return (
              <div key={wId} style={{padding:"10px 14px",background:"#FAF7F2",borderRadius:4,border:"1px solid rgba(15,26,46,.08)"}}>
                <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,fontWeight:600,color:"#66708A",letterSpacing:1.2,textTransform:"uppercase",marginBottom:8}}>
                  {wId === "(unknown)" ? "Unknown worksheet" : `Worksheet ${wId}`}
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:"6px 14px",fontFamily:"'IBM Plex Mono',monospace",fontSize:13,color:"#0F1A2E"}}>
                  {entries.map((e, i) => {
                    if(!e) return null;
                    ...
                    return (
                      <span key={i} style={{display:"inline-flex",alignItems:"center",gap:4}}>
                        <span style={{color:"#66708A"}}>{i+1}.</span>
                        <span style={{color:isBlank?"#9AA3B8":"#0F1A2E",fontStyle:isBlank?"italic":"normal",fontWeight:isBlank?400:600}}>
                          {isBlank ? "blank" : ans}
                        </span>
                        {flagIcon && <span style={{color:flagColor,fontWeight:700,marginLeft:2}}>{flagIcon}</span>}
                      </span>
                    );
                  })}
                </div>

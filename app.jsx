...
function makeConsultationStudent() {
  ...
  // WellEd Domain log entries — at least 3 per domain showing growth.
  const welledLogs = [];
  let logIdx = 0;
  ...
  // R&W domains (out of 27)
  pushLog("Reading & Writing", "Information & Ideas",        "easy",   15, 27,  14);
  pushLog("Reading & Writing", "Information & Ideas",        "medium", 19, 27,  60);
  pushLog("Reading & Writing", "Information & Ideas",        "hard",   22, 27, 140);
  pushLog("Reading & Writing", "Craft & Structure",          "easy",   17, 27,  21);
  pushLog("Reading & Writing", "Craft & Structure",          "medium", 20, 27,  75);
  pushLog("Reading & Writing", "Craft & Structure",          "hard",   23, 27, 155);
  pushLog("Reading & Writing", "Expression of Ideas",        "easy",   16, 27,  28);
  pushLog("Reading & Writing", "Expression of Ideas",        "medium", 18, 27,  90);
  pushLog("Reading & Writing", "Expression of Ideas",        "hard",   21, 27, 170);
  pushLog("Reading & Writing", "Standard English Conventions","easy",  19, 27,  35);
  pushLog("Reading & Writing", "Standard English Conventions","medium",22, 27, 105);
  pushLog("Reading & Writing", "Standard English Conventions","hard",  25, 27, 185);
  // Math domains (out of 22)
  pushLog("Math", "Algebra",                         "easy",   14, 22,  18);
  pushLog("Math", "Algebra",                         "medium", 17, 22,  72);
  pushLog("Math", "Algebra",                         "hard",   19, 22, 150);
  pushLog("Math", "Advanced Math",                   "easy",   13, 22,  24);
  pushLog("Math", "Advanced Math",                   "medium", 16, 22,  84);
  pushLog("Math", "Advanced Math",                   "hard",   18, 22, 162);
  pushLog("Math", "Problem-Solving & Data Analysis", "easy",   15, 22,  31);
  pushLog("Math", "Problem-Solving & Data Analysis", "hard",   18, 22, 110);
  pushLog("Math", "Problem-Solving & Data Analysis", "hard",   20, 22, 178);
  pushLog("Math", "Geometry & Trigonometry",         "easy",   13, 22,  38);
  pushLog("Math", "Geometry & Trigonometry",         "hard",   16, 22, 118);
  pushLog("Math", "Geometry & Trigonometry",         "hard",   19, 22, 192);

  // Practice exam scores — diagnostic + two follow-ups showing growth.
  const scores = [
    {
      id: "consult_score_1",
      ...
      mathScore: 510,
      isDiagnostic: true,
      notes: "Baseline practice exam.",
    },
    {
      id: "consult_score_2",
      date: dayShift(START, 100),
      testType: "WellEd Full — Practice Test 5",
      score: 1140,
      maxScore: 1600,
      rwScore: 580,
      mathScore: 560,
      notes: "Mid-program checkpoint, ~90 points up.",
    },
    {
      id: "consult_score_3",
      date: dayShift(START, 200),
      testType: "BlueBook Full — Practice Test 3",
      score: 1260,
      maxScore: 1600,
      rwScore: 640,
      mathScore: 620,
      notes: "Late-program practice; 210 pts above baseline.",
    },
  ];
...
function AppInner({authUser, onSignOut, currentUserEntry}){
  ...
  // ── Session 18C: Consultation Student seed ──────────────────────────────
  // Once the students listener has hydrated, check if the consultation
  // demo profile exists. If not, seed it. Idempotent: runs once per
  // session, no-ops on subsequent loads. Any authorized tutor/admin can
  // trigger creation — there's nothing tutor-specific about the data.
  ...
  useEffect(()=>{
    ...
    if(!students || students.length === 0) {
      // Empty roster — listener fired but found no docs. Still seed so
      // a brand-new project gets the demo profile.
    }
    if(students.find(st => st && st.id === CONSULTATION_STUDENT_ID)) {
    ...
    col.doc(CONSULTATION_STUDENT_ID).set({...data, id: CONSULTATION_STUDENT_ID})
      .then(()=> console.log("[consultation] seeded synthetic demo profile"))
      .catch((e)=> {
